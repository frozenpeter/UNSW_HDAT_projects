...
The Kaplan Meier curves shows there is a significant difference in survival time between ICU types and age groups. 
...
```{r task2.B_2 Multivariable Cox Models: Creating, Checking and Selection }
...
initial_cox_model <- coxph(Surv(Days, Status) ~ Age+ICUType+SAPS1+AST_max+Bilirubin_max+BUN_max+Creatinine_max+GCS_max+Urine_max+NISysABP_diff,
...
stepwise_cox_model1 <- summary(stepwise_cox_model)
stepwise_cox_model1
```

A model built will all the variables indicates that variables included in the model significantly contribute except for `Creatinine_max`. A reduction of the model was completed using the stepwise selection of AIC to improve the model. Further reductions were then assessed to see if a more parsimonious but equally reasonable model could be fit. 

```{r, 2B.3, }

#NISysABP_diff not significant in stepAIC model - decision to remove 

# Reduced Cox model 
cox_mod_reduced <- coxph(Surv(Days, Status)~Age + ICUType + SAPS1 + AST_max + Bilirubin_max + BUN_max + GCS_max + Urine_max, data=icu_patients_df1) 
cox_mod_reduced1 <- summary(cox_mod_reduced) 
cox_mod_reduced1

#review which is more parsimonious model 
AIC(stepwise_cox_model)
AIC(cox_mod_reduced)
#lower AIC with first model 

#assess if significant difference between models 
anova <- as.data.frame(anova(stepwise_cox_model, cox_mod_reduced))
#decision to stay with cox_mod_reduced model as more parsimonious with very similar concordance and AIC 


#Check whether the proportional hazards assumptions hold
cox.zph(cox_mod_reduced) # global p <0.01 - assumptions violated 
#ICUType, BUN_max, Creatinine_max - P.0.05 - hazards may be proportional over time in these groups 


```

The removal of `NISysABP_diff` was considered for the reduced model which remained a statistically significant model with all variables statistically significantly contributing. This reduced model had a lower AIC of  `r AIC(cox_mod_reduced)`, and ANOVA comparison of the two models was not significant (p= `r round(anova[2,4],3)`). Concordance remains reasonable at `r round(cox_mod_reduced$concordance[1],3)`. 

Given the incomplete data for this variable resulting in imputation of a moderate proportion of data, and the continued reasonable fit of the more parsimonious model it is reasonable to continue assessment with this reduced model. 

However, the proportional hazards of this model remains violated. Hence, assessment of whether this can be rectified stratification by days since ICU admission will be conducted. 


``` {r proportional hazards violation, eval = FALSE}
...
datasplit <- survSplit(Surv(Days, Status)~ Age + ICUType + SAPS1 + AST_max + Bilirubin_max + BUN_max + GCS_max , data=icu_patients_df1.2, cut=c(16), episode= "tgroup", id="id2")
summary(icu_patients_df1)

#
cox_mod_split1 <- coxph(Surv(Days, Status)~Age:strata(tgroup)+ICUType+SAPS1:strata(tgroup)+AST_max:strata(tgroup)+Bilirubin_max:strata(tgroup)+BUN_max+Creatinine_max+GCS_max:strata(tgroup)+Urine_max:strata(tgroup), data=datasplit)
summary(cox_mod_split1)                         
...
#### Model using unimputed dataset 


```{r,  unimputed data 2B, echo = FALSE}
# Missing values assessment of selected factors
sapply(icu_patients_df1, function(x) sum(is.na(x)))[
...
# Remove further data issues - inifite values
...
str(icu_patients_df0_r_missing_factors)




# Fit the full model again on the dataset without missing variables
initial_cox_model_df0 <- coxph(Surv(Days, Status) ~ Age + ICUType + SAPS1 + SOFA + BUN_max + Creatinine_max + GCS_max + Glucose_max + HCO3_max + HR_diff + Na_diff + pH_diff + Temp_diff + Urine_max + NISysABP_diff,
                          data = icu_patients_df0_r_missing_factors)
summary(initial_cox_model_df0)

# Fit the reduced model again on the dataset without missing values
