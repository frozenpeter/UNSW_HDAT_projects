...
```{r Step 2 EDA A Overall Demographics, fig.height=6, fig.width=13, echo=FALSE}
...
```{r Step 2 EDA A clinical measures, fig.width=10, echo=FALSE}
...
intact_model_rf <- glm(in_hospital_death ~ Age + SAPS1 + SOFA + Albumin_max  + ALT_max + AST_max + Bilirubin_max + BUN_max  + Creatinine_max + GCS_max + GCS_min + HR_diff + Lactate_max + Na_diff + NISysABP_diff + PaO2_max + pH_diff  + Temp_diff + Urine_max + WBC_max + ICUType, family = binomial(link = "logit"), data = icu_patients_df1_cleaned)
```
...
```
After check influential_point 1175, we find that:<br>
...
```{r task2.A_1 EDA1, fig.height=4, fig.width=4.5, echo=FALSE}
...
```{r task2.A_1 EDA2 MK, fig.height=8, fig.width=16, echo=FALSE}
...
```{r task2.B_3 diagnostic plots and Stepwise Cox Model, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hold',fig.height=4.5, fig.width=4.5}
...
```{r task2.B_4 Assess Model Assumptions, fig.height=8, fig.width=12, echo = FALSE}
...
```{r task2.B_6 Fit Cox model to df0 data, include = FALSE}
...
# Perform stepwise selection based on AIC 
#stepwise_cox_model_df0 <- stepAIC(initial_cox_model_df0, direction = "both")
#kable(summary(stepwise_cox_model_df0)$coefficients, caption = "Summary of Stepwise Cox Model")

```
**Please Note**: When Perform stepwise selection on re-fit Cox model, the error "number of rows in use has changed: remove missing values?" occured, as the data used for the Cox model fitting contains missing values.
...
```{r  task2.B_8 , echo = FALSE}
# Split the data at 1200 days
icu_patients_df0_impute$time_group <- ifelse(icu_patients_df0_impute$Days <= 1200, "Early", "Late")

# Fit a Cox model for each time group separately
...
# Extract the summary early
summary_early <- summary(cox_model_early)

# Create a data frame with variable names, coefficients, and p-values
result_early <- data.frame(
  Variable = rownames(summary_early$coefficients),
  Coefficient = summary_early$coefficients[, "coef"],
  `p-value` = summary_early$coefficients[, "Pr(>|z|)"]
)

#print(result_early)

# Extract the summary late
summary_late <- summary(cox_model_late)

# Create a data frame with variable names, coefficients, and p-values
result_late <- data.frame(
  Variable = rownames(summary_late$coefficients),
  Coefficient = summary_late$coefficients[, "coef"],
  `p-value` = summary_late$coefficients[, "Pr(>|z|)"]
)

#print(result_late)
# Assuming result_early and result_late are already created and have the same rows in the same order
combined_results <- cbind(result_early, result_late)

# Optionally, you might want to rename the columns to reflect which results are from the early or late period
colnames(combined_results) <- c("Variable_Early", "Coefficient_Early", "P_Value_Early", "Variable_Late", "Coefficient_Late", "P_Value_Late")
...
Choosing a Split Point at 1200 days (close to median), adjusting the dataset into Early Phase and Late Phase and fit separate Cox models for each time interval.
#### Early Phase:
...
# Fit a Cox model
cox_model <- coxph(surv_obj ~ Age + BUN_max + Creatinine_max, 
                   data = icu_patients_df0_impute)
#summary(cox_model)

# Fit Weibull model using flexsurv package
weibull_model <- flexsurvreg(Surv(Days, Status) ~ Age + BUN_max + Creatinine_max, data = icu_patients_df0_impute, dist = "weibull")

#print(summary(weibull_model))

# Fit Gompertz model using flexsurv package
gompertz_model <- flexsurvreg(Surv(Days, Status) ~ Age + BUN_max + Creatinine_max, data = icu_patients_df0_impute, dist = "gompertz")
...
# Fit the Piecewise Constant Hazards model using eha::pchreg
pch_model <- eha::pchreg(surv_obj ~ Age + BUN_max + Creatinine_max, 
                         data = icu_patients_df0_impute,
                         cuts = time_cuts)
#print(summary(pch_model))
...
# Fit a Cox model
cox_model_fl <- coxph(surv_obj ~ Age + SAPS1 + SOFA + BUN_max + Creatinine_max + GCS_max + GCS_min + Urine_max + ICUType, 
                   data = icu_patients_df0_impute)


# Fit Weibull model using flexsurv package
weibull_model_fl <- flexsurvreg(Surv(Days, Status) ~ Age + SAPS1 + SOFA, data = icu_patients_df0_impute, dist = "weibull")

# Fit Gompertz model using flexsurv package
gompertz_model_fl <- flexsurvreg(Surv(Days, Status) ~ Age + SAPS1 + SOFA, data = icu_patients_df0_impute, dist = "gompertz")


# Fit the Piecewise Constant Hazards model using eha::pchreg
pch_model_fl <- eha::pchreg(surv_obj ~ Age + SAPS1 + SOFA + BUN_max + Creatinine_max + GCS_max + GCS_min + Urine_max + ICUType,
                         data = icu_patients_df0_impute,
                         cuts = time_cuts)


# Fit a Cox model for baseline hazard comparison
check.dist(cox_model_fl, weibull_model_fl)
check.dist(cox_model_fl, gompertz_model_fl)
check.dist(cox_model_fl, pch_model_fl)
